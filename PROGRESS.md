...
- **2026-01-15**: TASK-222 ✅ - Drag-and-drop between folders. Implemented full drag-and-drop functionality for moving flight plans between folders: (1) Added drag data types and interfaces (`FlightPlanDragData`, `FLIGHT_PLAN_DRAG_TYPE` mime type) to `FlightPlanCard.tsx`. (2) Updated `FlightPlanCard` with `draggable`, `onDragStart`, `onDragEnd` props, visual feedback during drag (opacity, dashed border, scale transform), drag handle grip icon (6-dot pattern), and cursor changes (grab/grabbing). (3) Updated `FlightPlanList` to pass drag props through to cards with `folderId` context. (4) Updated `FolderCard` as drop target with `onDragOver`, `onDragEnter`, `onDragLeave`, `onDrop` handlers, visual feedback when dragging over (blue highlight border/background, "Soltar aquí" indicator). (5) Updated `FolderList` to propagate drag and drop props. (6) Updated `FlightPlansUploader` with: `handleMovePlan` callback to update `folderId` via API, orphan plans section as drop zone (allows removing plans from folders by dropping), `isDraggingOverOrphans` state for visual feedback, drag handlers for all plan cards. (7) Added `moving` to `loadingPlanIds` for loading state tracking. Users can now drag any flight plan card and drop it onto a folder to move it, or drop onto "Planes sin carpeta" section to remove from folder. **All 223 tasks are now complete! Phase 8 Post-Launch QA is 100% finished.**
- **2026-01-15**: TASK-216 ✅ - Integrate ScanPatternGeneratorV2 with refs for stable handlers. Fixed SCAN mode click handling by using refs instead of state callbacks to prevent stale closure issues. Changes: (1) In `ScanPatternGeneratorV2.tsx`: removed separate click handlers per step, created single `stableMapClickHandler` using `useCallback` with empty deps that reads all state from refs (`stateRef`, `serviceBoundsRef`), handler dispatches to correct step logic based on `currentStep` from ref, added `hasActiveClickTarget()` helper for determining when click handling is needed. (2) In `PlanGenerator.tsx`: replaced `useState` for `scanMapClickHandler` with `useRef` (`scanMapClickHandlerRef`), added `scanHandlerVersion` state to trigger re-renders when handler changes, created stable `handleSetScanMapClickHandler` callback, pass ref value to PlanMap with version key for proper updates. This approach ensures the map click handler identity remains stable across React re-renders, preventing the cleanup/re-registration cycle that was causing clicks to be lost between step transitions. Build and all 212 tests pass.
- **2026-01-15**: TASK-223 ✅ - Improve desktop layout width. Enhanced the desktop experience by adding proper width constraints and centering for main content areas. Changes: (1) Updated `FlightPlansUploader.tsx` to use a max-width container (`max-w-7xl`) with responsive horizontal padding (`px-4 sm:px-6 lg:px-8`) and vertical padding (`py-6`), ensuring content is properly centered and doesn't stretch excessively on wide screens. (2) Updated `FlightPlansUploaderDev.tsx` with the same improvements - changed from `p-6` inline padding to container-based layout with `max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6`. (3) Updated main `page.tsx` to use consistent `max-w-7xl mx-auto` container instead of the default Tailwind `container`. (4) Updated `trajectory-generator/page.tsx` wrapper to use full-width while allowing child components to control their own max-width. The 1280px (7xl) max-width ensures content remains readable on ultrawide monitors while providing ample space on standard desktop displays. Build and all 212 tests pass.
- **2026-01-15**: TASK-221 ✅ - Larger editable plan names. Enhanced `FlightPlanCard.tsx` with inline name editing capability: (1) Increased plan name font from `text-sm` to `text-lg` for better visibility. (2) Added `PencilIcon` SVG component that appears on hover when rename is enabled. (3) Implemented inline editing state management (isEditing, editName, validationError) with useRef for input focus. (4) Created `validateName()` function enforcing non-empty names (max 100 chars). (5) Added `handleRenameSubmit()` that calls onRename callback and shows toast on success/error. (6) Added keyboard support (Enter to save, Escape to cancel). (7) Created edit form with input field, save/cancel buttons, and validation error display. (8) Updated `FlightPlanList.tsx` with `onRenamePlan` prop and `renaming` loading state. (9) Updated `FolderCard.tsx` to pass rename handler to FlightPlanList. (10) Updated `FolderList.tsx` to accept and propagate onRenamePlan. (11) Enhanced `FlightPlansUploader.tsx` with `handleRenamePlan` callback using `updateFlightPlan()` with customName field, added renaming to loadingPlanIds state, connected rename throughout component tree. Users can now click the pencil icon on any flight plan card to edit its name inline with full validation and loading feedback.
