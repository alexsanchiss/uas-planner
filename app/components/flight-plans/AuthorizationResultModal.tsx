...
/**
 * Extract outer ring from geometry as [lng, lat] pairs (Cesium-ready).
      ...
      try {
        ...
        if (useWsGeozones) {
          for (const gz of geozonesSnapshot) {
            ...
            // Use robust ring extraction with coordinate order detection
            const ring = extractGeozoneRingForCesium(geom as { type: string; coordinates: unknown })
            if (!ring || ring.length < 3) continue

            const degreesArray: number[] = []
            ring.forEach((c: number[]) => {
              degreesArray.push(c[0], c[1]) // already [lng, lat] from helper
          ...
          for (const gz of geozones) {
            // Use robust ring extraction with coordinate order detection
            const ring = extractGeozoneRingForCesium(gz.geometry as { type: string; coordinates: unknown })
            if (!ring || ring.length < 3) continue
