...
\title{ SPIRIT \\ \small{ICD - V\_{2.4.0}}}
...
\subsubsection*{Sprint 4 --- Bug Fixes, UX Polish \& Nuevas Funcionalidades}
\begin{itemize}
    \item \textbf{Fix popup waypoint manual --- OK cierra popup, Enter dispara OK} (Feature 27): El botón OK en popups de edición de waypoints ahora cierra el popup tras confirmar cambios. La tecla Enter dispara la misma lógica que OK. Aplicado también a todos los popups SCAN (takeoff, landing, vértices polígono).
    \item \textbf{Mejoras SCAN pattern --- Add Vertex, bounds efectivos, validación drag} (Feature 28): Nuevo botón ``Add Vertex'' en paso 2 del sidebar SCAN; los límites del servicio usan \texttt{effectiveServiceLimits} (U-space seleccionado) en lugar de constantes globales; la operación de arrastrar vértices, takeoff y landing se valida contra los bounds del servicio.
    \item \textbf{Segundo click en plan seleccionado abre modal de waypoints} (Feature 29): Al hacer click en un plan ya seleccionado, se abre el modal de previsualización de waypoints en lugar de deseleccionar el plan.
    \item \textbf{Fix actualización de color de volúmenes 4D durante Play} (Feature 30): Se reemplaza la asignación directa de \texttt{material} y \texttt{outlineColor} en entidades Cesium por \texttt{ColorMaterialProperty} y \texttt{ConstantProperty}, lo que permite que Cesium detecte el cambio y re-renderice correctamente durante la animación.
    \item \textbf{Fix polilínea de trayectoria 3D --- eliminado terreno AMSL} (Feature 31): Se elimina el muestreo de terreno (\texttt{sampleTerrainMostDetailed}) del visor \texttt{Trajectory3DViewer}; la polilínea usa directamente altitudes AGL del CSV, unificando la referencia de altura con waypoints y dron.
    \item \textbf{Visor 3D de Geoawareness con geozonas extruidas} (Feature 32): Nuevo componente \texttt{Geoawareness3DModal} basado en CesiumJS que muestra geozonas como polígonos 3D extruidos coloreados por tipo de restricción (\texttt{GEOZONE\_COLORS}), trayectoria como polilínea azul y volúmenes de operación. Las geozonas son clickables y muestran información de autoridad, restricciones y aplicabilidad en un InfoBox. Modal de Geoawareness 2D ampliado (85vw/1600px). Botón ``3D Geoawareness'' en la barra de herramientas de Plan Authorization.
    \item \textbf{Auto-relleno de email en formulario U-Plan} (Feature 33): El campo \texttt{contactDetails.emails[0]} del formulario U-Plan se pre-rellena automáticamente con el email del perfil del usuario activo.
    \item \textbf{Sistema de borradores U-Plan} (Feature 34): Nuevo modelo Prisma \texttt{uplanDraft} para persistir borradores de U-Plan vinculados al usuario. Endpoints CRUD completos: \texttt{GET/POST /api/user/drafts} y \texttt{GET/PATCH/DELETE /api/user/drafts/[id]}. Sección ``U-Plan Drafts'' en la página \texttt{/profile} para listar, crear, editar nombre y eliminar borradores. El formulario \texttt{UplanFormModal} incluye desplegable ``Load Draft'' y botones ``Save as Draft'' / ``Update Draft''.
    \item \textbf{Eliminación de sección Flight Plan Details de Plan Definition} (Feature 35): Se eliminan $\sim$840 líneas del componente \texttt{PlanGenerator} (estado \texttt{flightPlanDetails}, botón ``Flight Plan Details'' y todo su bloque de contenido), simplificando el flujo de definición de plan.
\end{itemize}

\subsubsection*{Sprint 5 --- V2.2.1 Bug Fixes, UX \& Nuevas Funcionalidades}
\begin{itemize}
    \item \textbf{Fix overflow de tooltips en formulario U-Plan} (Task 1): Los tooltips del formulario ahora se renderizan mediante un portal, evitando el recorte por contenedores con \texttt{overflow:hidden}.
    \item \textbf{Respuestas FAS 400 establecen estado ``withdrawn''} (Task 2): Si FAS responde con HTTP~400, el U-Plan pasa a estado \texttt{withdrawn} y se envía email de denegación al usuario.
    \item \textbf{Centrado de mapa en waypoints manuales} (Task 3): Al añadir un waypoint mediante coordenadas manuales en Plan Definition, el mapa se centra automáticamente en la nueva posición.
    \item \textbf{Fix cámara de seguimiento y proximidad de etiquetas en visor 3D} (Task 4): Corregida la cámara de seguimiento del dron y la distancia de las etiquetas de waypoints en \texttt{Trajectory3DViewer}.
    \item \textbf{Timeout de inactividad de 10 minutos} (Task 5): Se añade un overlay de ``Sesión expirada'' tras 10~minutos de inactividad del usuario, requiriendo re-login.
    \item \textbf{Eliminación del botón auto-play en visor 3D de U-Plan} (Task 6): Se elimina el botón de reproducción automática del visor 3D de línea temporal de U-Plan.
    \item \textbf{Restricción de fechas pasadas en date picker} (Task 7): El selector de fecha de programación de vuelo impide seleccionar fechas anteriores al momento actual.
    \item \textbf{Reset de plan aprobado cancela autorización FAS} (Task 8): Resetear un plan aprobado envía cancelación a FAS y notificación por email al usuario, con modal de confirmación.
    \item \textbf{Bloqueo de reset para U-Plans externos} (Task 9): Los U-Plans subidos externamente no pueden resetearse; se muestra mensaje informativo.
    \item \textbf{U-Plans externos omiten recompilación de volúmenes} (Task 10): Al pulsar ``Save \& Request Auth'' en un U-Plan externo se envía directamente a FAS sin regenerar volúmenes.
    \item \textbf{Fix overflow vertical en popup de geozonas} (Task 11): El popup de información de geozonas en Plan Definition ahora es scrollable verticalmente, evitando desbordamiento.
    \item \textbf{Fix renderizado 3D de geozonas} (Task 12): Corregido el renderizado de geozonas 3D en los modales de Geoawareness y Resultado de Autorización.
    \item \textbf{Eliminación de adjuntos JSON en emails de autorización} (Task 13): Los emails de resultado de autorización ya no incluyen el JSON del U-Plan como adjunto.
    \item \textbf{Bump de versión y actualización de documentación} (Task 14): Versión actualizada a 2.2.1; documentación ICD actualizada con changelog del Sprint~5.
    \item \textbf{Nueva splash page AETHON con shader animado} (Task 15): Nueva página de bienvenida con fondo de shader animado WebGL y enlace a \texttt{/launch}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Versión de la aplicación}: \texttt{v2.2.0 SPIRIT} (Sprints 1--5: 42 funcionalidades, proxy SSE de Geoawareness, visores 3D completos, gestión de usuario, sistema de contacto, restricciones EU, borradores U-Plan, visor Geoawareness 3D, correcciones UX y altimetría AGL en Geoawareness 3D).
    ...
    \begin{itemize}
        ...
        \item \textbf{Nuevos endpoints (Sprints 1--3)}: \texttt{GET/PATCH /api/user/profile}, \texttt{POST /api/contact}, \texttt{GET /api/cesium/token}, \texttt{GET /api/geoawareness/stream/[uspaceId]}.
        \item \textbf{Sprint 4 --- Bug Fixes, UX Polish \& Nuevas Funcionalidades}:
        \begin{itemize}
            \item Fix popup waypoint (OK cierra, Enter confirma); mejoras SCAN (Add Vertex, bounds efectivos, drag validado); segundo click en plan abre waypoints.
            \item Fix color 4D (\texttt{ColorMaterialProperty}/\texttt{ConstantProperty}); fix polilínea trayectoria 3D (eliminado AMSL, unificado AGL).
            \item Nuevo \texttt{Geoawareness3DModal} con geozonas 3D extruidas, trayectoria y volúmenes.
            \item Auto-relleno email U-Plan desde perfil; sistema de borradores U-Plan (\texttt{uplanDraft} + endpoints CRUD).
            \item Eliminación de sección Flight Plan Details de Plan Definition ($\sim$840 líneas).
        \end{itemize}
        \item \textbf{Sprint 5 --- Correcciones de visores 3D, gestión de borradores y UX}:
        \begin{itemize}
            \item \textbf{Fix preview waypoints en panel de plan seleccionado} (F37): Prop \texttt{onWaypointPreviewClick} propagada al componente \texttt{FlightPlanCard} en el panel lateral; click en plan seleccionado abre correctamente el modal de waypoints.
            \item \textbf{Fix renderizado continuo en \texttt{Cesium3DModal}} (F38): Se activa \texttt{viewer.scene.requestRenderMode = false} inmediatamente tras la creación del viewer, garantizando actualización continua de la escena para animaciones y carga de terreno.
            \item \textbf{Eliminación de polilínea de trayectoria en visores 3D} (F39): Reemplazada la polilínea continua de trayectoria en \texttt{Trajectory3DViewer} y \texttt{Geoawareness3DModal} por marcadores individuales de puntos: despegue (verde), aterrizaje (rojo) y waypoints intermedios (azul); se conservan etiquetas en despegue/aterrizaje.
            \item \textbf{Fix tema oscuro en InfoBox/widgets de Cesium} (F40): Inyección de CSS con \texttt{background: \#1e1e1e !important; color: \#e0e0e0 !important;} en el \texttt{<iframe>} de InfoBox y en los contenedores de widgets externos de los cinco visores CesiumJS (\texttt{Cesium3DModal}, \texttt{Denial3DModal}, \texttt{AuthorizationResultModal}, \texttt{Trajectory3DViewer}, \texttt{Geoawareness3DModal}).
            \item \textbf{Gestión de borradores U-Plan mejorada} (F41): Botón ``Eliminar borrador'' (rojo + icono de papelera) añadido al formulario \texttt{UplanFormModal}; botones ``Actualizar'' (teal) y ``Guardar como borrador'' (verde) con iconos diferenciados; eliminada opción de borrar borrador desde la página de Perfil (se centraliza en el formulario).
            \item \textbf{Auto-zoom de cámara a zona de vuelo en Geoawareness 3D}: La cámara en \texttt{Geoawareness3DModal} hace zoom al BoundingSphere de volúmenes de operación + trayectoria (en lugar de la totalidad de geozonas), mostrando el área de vuelo relevante al abrir el modal.
            \item \textbf{Fix altimetría AGL en volúmenes de operación en Geoawareness 3D} (F42): Los volúmenes de operación de U-Plan en \texttt{Geoawareness3DModal} se convierten de AGL a AMSL mediante \texttt{Cesium.sampleTerrainMostDetailed()} aplicado al centroide de cada polígono; se eliminan \texttt{heightReference: RELATIVE\_TO\_GROUND} y \texttt{extrudedHeightReference: RELATIVE\_TO\_GROUND} a favor de alturas absolutas AMSL calculadas explícitamente (\( h_{\text{AMSL}} = h_{\text{terreno}} + h_{\text{AGL}} \)); esto corrige la aparición de volúmenes a nivel del mar cuando las teselas de terreno no han cargado al inicializar el viewer.
        \end{itemize}
        \item \textbf{Nuevos endpoints (Sprint 4)}: \texttt{GET/POST /api/user/drafts}, \texttt{GET/PATCH/DELETE /api/user/drafts/[id]}.
    \end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{GET }\icdendpoint{/api/user/drafts} (v2.2.0 Sprint 4: lista borradores U-Plan del usuario)
    \item \texttt{POST }\icdendpoint{/api/user/drafts} (v2.2.0 Sprint 4: crea un borrador U-Plan)
    \item \texttt{GET }\icdendpoint{/api/user/drafts/[id]} (v2.2.0 Sprint 4: obtiene un borrador específico)
    \item \texttt{PATCH }\icdendpoint{/api/user/drafts/[id]} (v2.2.0 Sprint 4: actualiza nombre o datos de un borrador)
    \item \texttt{DELETE }\icdendpoint{/api/user/drafts/[id]} (v2.2.0 Sprint 4: elimina un borrador)
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{Geoawareness3DModal}: Visor 3D de geozonas extruidas con trayectoria y volúmenes (v2.2.0 Sprint 4).
...
\subsubsection*{Modelo de datos adicional (v2.2.0 Sprint 4)}
\begin{itemize}
    \item \texttt{uplanDraft}: Modelo Prisma para borradores de U-Plan. Campos: \texttt{id} (autoincremental), \texttt{userId} (FK $\rightarrow$ user), \texttt{name} (varchar), \texttt{draftData} (JSON), \texttt{createdAt}, \texttt{updatedAt}. Índice en \texttt{userId}; cascada en eliminación del usuario.
\end{itemize}
...
% ============================================================
\subsection*{Novedades en V2.4.0}

\subsubsection*{Plan Activation — Flujo completo de activación de vuelo}

\begin{itemize}
  \item \textbf{Nuevos campos en \texttt{flightPlan}}: \texttt{activationStatus}, \texttt{activatedAt}, \texttt{activationMessage}, \texttt{termsAcceptedAt}, \texttt{lastActivationAttempt}. Estados posibles de \texttt{activationStatus}: \textit{no activable}, \textit{activable}, \textit{activando}, \textit{autorizado\_despegue}, \textit{denegado\_activacion}, \textit{ventana\_pasada}.
  \item \textbf{\texttt{GET /api/flightPlans/[id]/terms}}: Proxy autenticado al FAS. Devuelve los T\&C del plan aprobado bajo demanda. Requiere \texttt{authorizationStatus = aprobado} y \texttt{externalResponseNumber} presente.
  \item \textbf{\texttt{POST /api/flightPlans/[id]/activate}}: Valida ventana $\pm 60\,\text{s}$ respecto a \texttt{scheduledAt}, cooldown de 5 s entre intentos, y \texttt{termsAccepted = true} en el cuerpo. Llama a \texttt{GET /activation/[externalResponseNumber]} en el FAS con timeout de 10 s. Persiste resultado en \texttt{activatedAt} / \texttt{activationStatus}.
  \item \textbf{\texttt{GET /api/flightPlans/active}}: Lista planes aprobados con \texttt{scheduledAt} en $[\text{now}-1\text{h},\,\text{now}+24\text{h}]$.
  \item \textbf{\texttt{GET /api/flightPlans/history}}: Lista paginada de planes históricos (activados, ventana pasada, denegados/retirados). Parámetros \texttt{limit} (máx.\ 100) y \texttt{offset}.
  \item \textbf{Página \texttt{/plan-activation}}: Sección ``Vuelos activables'' (filtrado $\pm 60\,\text{s}$ en cliente con tick de 1 s) y sección ``Historial'' colapsable con paginación.
  \item \textbf{\texttt{ActivateFlightModal}}: Carga T\&C del FAS en \texttt{JsonViewerSections}, exige aceptación explícita, ejecuta la activación y muestra ``PUEDE DESPEGAR'' durante 60 s tras respuesta positiva.
\end{itemize}

\subsection*{Novedades en V2.3.0}
% ============================================================

\subsubsection*{Correcciones y estabilidad}
\begin{itemize}
    \item \textbf{Eliminación del cierre de sesión por inactividad} (\texttt{fix(auth)}): Se elimina completamente la lógica de \textit{inactivity timeout} (10~min) del hook \texttt{useAuth.ts}. La sesión ahora solo expira cuando el \textit{refresh token} caduca; no se cierran sesiones activas por ausencia de eventos de ratón/teclado.
    \item \textbf{Corrección del error SSR de Leaflet} (\texttt{fix(ui)}): El componente \texttt{DenialMapModal} importaba \texttt{react-leaflet} directamente, provocando el error \texttt{window is not defined} en el servidor. Se aisla la lógica del mapa en \texttt{DenialMapContent.tsx} con importaciones dinámicas (\texttt{ssr: false}), siguiendo el patrón ya presente en \texttt{TrajectoryMapViewer} y \texttt{GeoawarenessViewer}.
\end{itemize}

\subsubsection*{Propuesta de trayectoria alternativa SCRS}
\begin{itemize}
    \item \textbf{Parser de mensajes SCRS} (\texttt{lib/scrs.ts}): Nuevo módulo que extrae la alternativa de trayectoria del campo \texttt{scr\_dispatch} del \texttt{authorizationMessage} del FAS. Guarda: \texttt{sent=true}, \texttt{status\_code=200}, \texttt{response.status='success'}, segmentos no vacíos. Convierte coordenadas GeoJSON $[\text{lon}, \text{lat}, \text{alt}]$ a \texttt{\{lat, lon, alt\}} y concatena los \texttt{route\_path} de todos los segmentos, deduplicando puntos en las costuras.
    \item \textbf{Builder QGC reutilizable} (\texttt{lib/qgc-plan.ts}): Extracción de la función de generación de planes \texttt{.plan} (QGroundControl) a una librería independiente (\texttt{buildQgcPlan}). El componente \texttt{PlanGenerator.tsx} se refactoriza para llamar a este helper. Permite reutilización desde el endpoint de aceptación.
    \item \textbf{Modal comparativo de trayectorias} (\texttt{AlternativeTrajectoryModal}): Nuevo componente SSR-safe que muestra la trayectoria actual (polilínea azul) y la propuesta por el SCRS (polilínea verde discontinua) sobre un mapa Leaflet. Incluye \texttt{FitBoundsHandler}, estadísticas de waypoints y botones \textit{Rechazar} / \textit{Aceptar alternativa}.
    \item \textbf{Endpoint de aceptación} (\texttt{POST /api/flightPlans/{id}/accept-alternative}): Cuando el usuario acepta la alternativa, el endpoint: (1) valida la alternativa SCRS; (2) construye la lista de waypoints (puntos SCRS + waypoint de aterrizaje final a 0~m); (3) regenera \texttt{fileContent} en formato QGC \texttt{.plan}; (4) actualiza \texttt{uplan.flightDetails.waypoints} y vacía \texttt{operationVolumes}; (5) ejecuta una transacción Prisma que borra el \texttt{csvResult} y reinicia el plan a \texttt{status='sin procesar'} preservando \texttt{scheduledAt} y los metadatos del U-Plan.
    \item \textbf{Integración en UI} (\texttt{FlightPlansUploader}): El modal de resultado de autorización muestra un botón \textit{Ver alternativa} (verde) solo cuando el plan está denegado y el mensaje contiene un \texttt{scr\_dispatch} válido. El flujo de aceptación llama al nuevo endpoint, muestra toast de confirmación y refresca la lista de planes.
    \item \textbf{Compatibilidad del flujo de reautorización}: El plan reprocesado sigue el pipeline estándar (\textit{sin procesar} $\rightarrow$ \textit{en cola} $\rightarrow$ \textit{procesado} $\rightarrow$ autorización FAS $\rightarrow$ email de resultado), sin cambios en los componentes existentes.
\end{itemize}
